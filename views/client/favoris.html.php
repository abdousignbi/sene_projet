<?php 
/*  if (isset($_SESSION['arrayFavoris'])){
    $arrayFavoris=$_SESSION['arrayFavoris'];
   // var_dump($arrayError);
    unset($_SESSION['arrayFavoris']);
}  */ 
...
?>
  ...
  <h3 class="abdou" style="margin-left:40%;color:#f17503;margin-top:-10%;">Liste des produits</h3>
     ...
     if (!isset($_GET['page'])) {
      $page=1;
      $_SESSION['arrayFavoris'] = $arrayFavoris;
      $nombrePage=nombrePageTotal($_SESSION['arrayFavoris'],6);
      $arrayFavori=get_element_to_display(  $_SESSION['arrayFavoris'],$page,6);  
      }
      if (isset($_GET['page'])) {
        $page=$_GET['page'];
        $ok1=$ok1;
...
<?php foreach ($arrayFavori as $key => $array):?>
    ...
    <td class="gueye" style="border:2px solid black;"><?php echo $_SESSION['userConnect']['id_user'];?></td>
